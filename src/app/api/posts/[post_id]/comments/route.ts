...
export async function POST(req: NextRequest, { params }: { params: { post_id: string } }) {
  ...
  const created = await addComment(auth.userId, params.post_id, parsed.data.content);
  const post = await getPostById(params.post_id);
  if (post && post.author_id !== auth.userId) {
    await createNotification({ recipient: post.author_id, sender: auth.userId, type: 'comment', post_id: params.post_id, message: 'commented on your post' });
  }
